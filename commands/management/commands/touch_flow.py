...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        base_dir = os.path.dirname(
            ...
            )  # dirname removes last address from str : x\cc => x\ - we have four ladders commands/mg/commands/HEre \
        )
        ...
        def get_items(root_dir):
            ...
            for root, dirs, files in os.walk(root_dir):
                rel = os.path.relpath(
                    root, root_dir
                )  # GOES THROUGH EACH FOLDER AND PRINTS REALIVE PATH (root) , DIRECTOREIS AND FILES INSIDE OF IT>
                if rel == ".":
                    rel = ""
        ...
        def get_icon(name, is_dir):
            ...
            if ext in (".mp4", ".mov", ".avi"):
                return "🎬"
            if ext in (".yml", ".yaml", ".json", ".xml", ".toml"):
                return "📝"
        ...
        with open(
            os.path.join(structure_dir, "flowchart-raw.txt"), "w", encoding="utf-8"
        ) as f:
            f.write("\n".join(raw_lines))
        ...
        icon_lines.insert(0, "📁")
        with open(
            os.path.join(structure_dir, "flowchart.txt"), "w", encoding="utf-8"
        ) as f:
            f.write("\n".join(icon_lines))

        self.stdout.write(
            self.style.SUCCESS(f"✅ flowcharts updated in {structure_dir}")
        )
